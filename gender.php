...
function getGenderFromName($fullName) {
    ...
    if (mb_substr ($array['surename'], $lenghtSurename - 2) === 'ва') $failure --;   
    if (mb_substr ($array['name'], $lenghtName - 1) === 'й' || mb_substr ($array['name'], $lenghtName - 1) == 'н' ) $failure ++;
    if (mb_substr ($array['name'], $lenghtName - 1) === 'а') $failure --;
    if (mb_substr ($array['patronomyc'], $lenghtPatronomyc - 2) === 'ич') $failure ++;
    if (mb_substr ($array['patronomyc'], $lenghtPatronomyc - 3) === 'вна') $failure --;
    return $failure <=> 0;
};